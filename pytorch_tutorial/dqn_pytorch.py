...
class DeepQNetwork(nn.Module):
    def __init__(self, lr, input_dims, fc1_dims, fc2_dims, n_actions):
        ...
        self.fc1_dims = fc1_dims #hidden dimeantions
...
class Agent():
    def __init__(self, gamma, epsilon, lr, input_dims, batch_size, n_action, max_mem_size= 100000, eps_end = 0.01, eps_dec= 5e-4):
        ...
        self.Q_eval = DeepQNetwork(self.lr, n_actions=n_action, input_dims=input_dims,
        ...
        #model free, bootstapped (estimates), off-policy (eps-greedy)
        self.action_memory=np.zeros(self.mem_size, dtype=np.int32)
        self.reward_memory= np.zeros(self.mem_size, dtype=np.float32)
        self.terminal_memory = np.zeros(self.mem_size, dtype=np.bool)

    def store_transition(self, state, action, reward, new_state, done):
        index = self.mem_cntr% self.mem_size #rewrite agent's earliest memory
    ...
    def choose_action(self, observation):
        if np.random.random()>self.epsilon:
            state = T.tensor([observation]).to(self.Q_eval.device) #Take observation, build it to tensor and send it to our device
